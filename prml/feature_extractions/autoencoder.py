...
#            self.parameter[f"b_decode{i}"] = nn.Parameter(np.zeros(args[i]))
            self.parameter["w_encode{}".format(i)] = nn.Parameter(np.random.randn(args[i], args[i + 1]))
            self.parameter["b_encode{}".format(i)] = nn.Parameter(np.zeros(args[i + 1]))
            self.parameter["w_decode{}".format(i)] = nn.Parameter(np.random.randn(args[i + 1], args[i]))
            self.parameter["b_decode{}".format(i)] = nn.Parameter(np.zeros(args[i]))

    def transform(self, x):
        h = x
        for i in range(self.n_unit - 1):
            #h = nn.tanh(h @ self.parameter[f"w_encode{i}"] + self.parameter[f"b_encode{i}"])
            h = nn.tanh(h @ self.parameter["w_encode{i}".format(i)] + self.parameter["b_encode{i}".format(i)])
        return h.value
    ...
    def forward(self, x):
        ...
        for i in range(self.n_unit - 2, 0, -1):
            ...
            h = nn.tanh(h @ self.parameter["w_decode{i}".format(i)] + self.parameter["b_decode{i}".format(i)])
